...
from jittor_geometric.nn.models.dyrepstructure import DyRep
...
def mae_error(u, v, k, time_cur, expected_time, reoccur_dict, end_date):
    ...
    for idx in range(N):
        ui = int(u[idx])
        vi = int(v[idx])
        eti = int(et[idx]) # 确保 et[idx] 是可以转换为整数的

        key = (ui, vi, eti) if ui <= vi else (vi, ui, eti)
        val = reoccur_dict[key]
...
if __name__ == '__main__':
    ...
    print(args.lr_decay_step)
    # Set seed
    np.random.seed(args.seed)
    rnd = np.random.RandomState(args.seed)

    
    # jittor.flags.cudnn_deterministic = 1
    # jittor.flags.cudnn_benchmark = 1
    ...
    initial_embeddings = [jittor.randn(train_set.N_nodes, args.hidden_dim)]
    ...
    for epoch in range(epoch_start, args.epochs + 1):
        ...
        for batch_idx, data_batch in enumerate(tqdm(train_loader)):
            ...
            time_iter = time.time() - start
            # model.dz = model.dz.detach()  # to reset the computational graph and avoid backpropagating second time
        ...
        print(f"time this epoch：{epoch_elapsed_time:.6f} seconds")
    #
