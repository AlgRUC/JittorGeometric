...
SpmmcooOp::SpmmcooOp(Var* outputVar_, Var* x_, Var* row_indices_,Var* col_indices_,Var* value_,int A_row_,int A_col_,NanoString dtype_)
    : outputVar(outputVar_), x(x_),row_indices(row_indices_), col_indices(col_indices_), value(value_),A_row(A_row_),A_col(A_col_),dtype(dtype_) {
...
void SpmmcooOp::jit_run() {
    ...
    CHECK_CUSPARSE( cusparseCreateCoo(&matA, A_row, A_col, vs[0], row_indices->ptr<int>(), col_indices->ptr<int>(), value->ptr<T>(), CUSPARSE_INDEX_32I, CUSPARSE_INDEX_BASE_ZERO, CUDA_R_32F) );
