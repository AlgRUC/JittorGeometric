 ...
 * @Description: 
 * @Author: lusz
 * @Date: 2025-04-26 14:55:42
 */
#include "var.h"
#include "indexselect_op.h"

namespace jittor {
#ifndef JIT
    IndexselectOp::IndexselectOp(Var* outputVar_, Var* x_,int dim_,Var* index_) :
    ...
    void IndexselectOp::jit_prepare(JK& jk) {
        ...
        void IndexselectOp::jit_run() {
            ...
            for(int i=0;i<i_num;i++){
                for(int j=0;j<feature_dim;j++){
                    out_ptr[i*feature_dim+j]=x_ptr[i_ptr[i]*feature_dim+j];
                }
            }
        }
    ...
    __global__ void computeKernel(const Tint* __restrict__ i_ptr, 
        ...
        for (Tint idx = tid; idx < total; idx += blockDim.x * gridDim.x) {
            ...
            out_ptr[idx] = x_ptr[i_ptr[i] * feature_dim + j];
        }
        __syncthreads();
    }
